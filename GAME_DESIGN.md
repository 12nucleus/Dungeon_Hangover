...
## Premise
A turn-based roguelite dungeon crawler, playable in **first-person** or **isometric** view (toggle anytime, same simulation underneath). You wake up at the bottom of a 50-floor dungeon in your underwear, nursing a splitting headache, next to a bonfire and a bag containing basic suplies (rusty dagger, health potion, a torch). The only way out is up.
...
**Gameplay reference: Baldur's Gate 3.** Maximum interaction flexibility — environmental objects can be shoved, thrown, ignited, or used as weapons; skills can be applied outside combat to solve problems (persuade, intimidate, sneak, break things); positioning, verticality, and improvisation matter as much as raw stats. HUD similar ro BG3.

## Player Progression — No Fixed Classes
There is no class select screen. Instead:
- Leveling is actually sobbering up which grant a **choice of skills/perks** pulled from 15 ridiculous archetype pools (Bar bouncer, Gutter Rogue, Karaoke Bard, Somelier, etc) — the player builds their own hybrid over time.
...
1. **Procedural chaos, authored soul.** Room layout, loot rolls, and enemy placement are generated. NPC dialogue, quests, and companion arcs are hand-written, never generated at line-level.
2. **Comedy from situation, not from jokes.** A half-naked hungover idiot fighting a skeleton with a fish is funny because of what's happening, and funnier because a narrator is judging you for it in real time.
3. **Every death is fair, never a waste.** Death returns you to the last bonfire. Gold, loot, and levels persist — only positional progress is lost.
4. **Flexibility over fixed builds.** No class walls, no "wrong" way to solve a room — skills, items, and environment combine freely (BG3-style), and the game should reward creative misuse of objects.
...
- intro cutscene showing the hero in a tavern drinking and being beligerent..then passing out after a last drink...then waking up (must start laying on the ground), standing up with a migraing (a few star like particules to simulate headhache). Narration explaining the premise of the Hero. the hero name is Greg. 
- Floors 1–5, each dungeon floor is a different theme, different.
- 25-30 enemy types, 1 handcrafted boss on per floor.
- Hangover leveling, torch/brazier lighting, bonfire checkpoints.
...
- Expand to 50 themes across acts, 200 enemies types / 15 factions, all 50 bosses.
...
- Full narrator voice + TTS sub-voice acting (this is where "Dungeon Crawler Carl narrator" energy should really land). Use edge-TTS for voice acting, andrews voice.
...
## Loot Rules
- 10 tiers, scaling with dungeon depth and player level.
